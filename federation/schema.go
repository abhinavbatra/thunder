...
func validateFederationKeys(serviceNames []string, serviceSchemasByName map[string]*introspectionQueryResult, obj *graphql.Object, fName string) error {
	...
	if !validFederatedKey {
		return oops.Errorf("Invalid federation key")
	}
	return nil
}
				...
				for _, field := range typ.Fields {
					...
					for _, arg := range field.Args {
						...
						for fName := range inputType.InputFields {
							...
							err := validateFederationKeys(serviceNames, serviceSchemasByName, obj, fName)
							if err != nil {
								return nil, err
							}
